...
interface AuthContextValue {
  ...
  loading: boolean;
  /** True after the initial session check has resolved. */
  initialized: boolean;
  signIn: (email: string, password: string) => Promise<{ error: string | null }>;
  signUp: (email: string, password: string) => Promise<{ error: string | null; user: User | null }>;
  signOut: () => Promise<void>;
  /** Refresh the workspace association after onboarding completes. */
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const loadWorkspace = useCallback(async (uid: string | undefined) => {
    ...
    // Pull the user's first workspace membership + the workspace row itself
    // in a single query.
  ...
  useEffect(() => {
    ...
    client.auth.getSession().then(async ({ data: { session: existing } }) => {
      setSession(existing);
      setUser(existing?.user ?? null);
      if (existing?.user) await loadWorkspace(existing.user.id);
      setLoading(false);
      setInitialized(true);
    });
