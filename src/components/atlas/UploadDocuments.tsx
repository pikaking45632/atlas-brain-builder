      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-12">
        <div className="w-full max-w-2xl space-y-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-3">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-cyan-400 flex items-center justify-center mx-auto shadow-lg">
              <Cloud className="w-8 h-8 text-white" />
            </div>
            <h1 className="text-3xl font-display font-bold text-foreground">Upload Documents</h1>
            <p className="text-muted-foreground max-w-md mx-auto">
              Feed Atlas with your company policies, handbooks, procedures, and any documents you want your AI to understand.
            </p>
          </motion.div>
          ...
          <motion.div
            ...
            className={`relative border-2 border-dashed rounded-2xl p-12 text-center transition-all cursor-pointer ${
              dragActive
                ? "border-primary bg-primary/5 shadow-[0_0_30px_-5px_hsl(var(--primary)/0.2)]"
              ...
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="space-y-2">
                {files.map((file) => (
                  ...
                  >
                    ...
                    {file.status === "done" && <CheckCircle2 className="w-5 h-5 text-green-500 shrink-0" />}
                    {file.status === "error" && <span className="text-xs text-destructive font-medium">Failed</span>}
                    <button onClick={(e) => { e.stopPropagation(); removeFile(file.name); }} className="text-muted-foreground hover:text-foreground">
                      <X className="w-4 h-4" />
                    </button>
                  </motion.div>
                ))}
          ...
          <div className="flex justify-between pt-4">
            ...
            <Button onClick={() => setStep(7)} className="gap-2">
